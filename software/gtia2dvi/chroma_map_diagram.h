...
static inline void _plot_diag_pixel_big(int16_t dec, uint i, uint row, int color)
{
    int f = DEC_F(dec);
    int a = DEC_A(dec);
    int b = DEC_B(dec);
    ...
    int pxl_color = (color > 0) ? color * 16 + 6 : NO_MAPPING_PIXEL_COLOR;
    ...
    if (color > 0)
    {
        ...
        uart_log_putlnf("%03X %01d.%02d.%02d r:%d x:%d c:%d", dec, f, a, b, row % 2, i & 0x3, color);
...
static inline int _read_calibrated_color(int row, int modx, int enc_sample)
{
    return calibration_data[row][modx][enc_sample];
}

static inline void update_mapping_diagram(int16_t enc_sample)
{
    for (int modx = 0; modx < 4; modx++)
        for (int row = 0; row < 2; row++)
        {
            int color = _read_calibrated_color(row, modx, enc_sample);
            int pxl_color = (color > 0) ? color * 16 + 6 : NO_MAPPING_PIXEL_COLOR;
            plot_diag_pixel(enc_sample, modx, row, pxl_color);
        }
}

static inline void dump_mapping_diagram(int16_t enc_sample)
{
    for (int modx = 0; modx < 4; modx++)
        for (int row = 0; row < 2; row++)
        {
            int color = _read_calibrated_color(row, modx, enc_sample);
            _plot_diag_pixel_big(enc_sample, modx, row, color);
        }
}
